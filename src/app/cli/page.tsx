...
export default function CLIView() {
              ...
              content: (
                <div className="space-y-2 py-2">
                  ...
                  <div className="flex gap-1">
                    {[1,2,3,4,5,6,7,8].map(i => <div key={i} className="h-1 w-4 bg-[#00ff9d]" />)}
                    {[1,2].map(i => <div key={i} className="h-1 w-4 bg-zinc-800" />)}
                  </div>
                  <p className="text-xs italic text-zinc-500">Your "Financial SOUL" is aligned with the Southampton Waterfront Shards.</p>
                </div>
              ),
              timestamp: new Date()
            }
            ...
            response = {
              ...
              content: (
                <div className="space-y-3 py-2 border-l-2 border-[#00ff9d]/30 pl-4">
                  <p className="text-[#00ff9d] font-bold">ASSET: Southampton Waterfront Shards (SWS-01)</p>
              ...
              content: (
                <div className="space-y-2 py-2">
                  <div className="flex items-center gap-2 text-blue-400">
                    <Activity size={14} />
                    <span className="font-bold">AGENT_LOG: Market Intelligence Scan...</span>
                  </div>
                  <p className="text-xs text-zinc-400">Comparing Bristol (6.2%) vs Manchester (7.1%) vs Southampton (8.2%)...</p>
                  <p className="text-[#00ff9d] text-xs">RECOMMENDATION: Diversify 20% of capital into Manchester Core for stability, keep 80% in Southampton for growth alpha.</p>
                </div>
              ),
              timestamp: new Date()
            }
  ...
  return (
    <main className="min-h-screen bg-[#08080a] text-zinc-100 font-mono selection:bg-[#00ff9d]/30 overflow-hidden flex flex-col">
      {/* Header */}
      <nav className="p-4 border-b border-white/5 flex justify-between items-center bg-black/40 backdrop-blur-md">
        <Link href="/" className="flex items-center gap-4 group">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold tracking-tight uppercase">Lumina CLI</span>
          </div>
        </Link>
        <div className="flex items-center gap-6">
          <div className="hidden md:flex flex-col items-end">
            <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Connection</p>
            <p className="text-[10px] text-[#00ff9d] font-bold">STABLE // GOOSE_AGENT_V1</p>
          </div>
      ...
      <div className="p-6 bg-black/60 border-t border-white/5">
        <form onSubmit={handleCommand} className="flex items-center gap-3">
          <span className="text-[#00ff9d] font-bold">{USERNAME}@{HOSTNAME}:~$</span>
          <input
            ref={inputRef}
            autoFocus
            type="text"
          ...
          <div className="flex items-center gap-2 px-2 py-1 bg-white/5 rounded border border-white/10">
            ...
            <span className="text-[10px] font-bold uppercase text-zinc-500">Agent Active</span>
          </div>
        </form>
      </div>
